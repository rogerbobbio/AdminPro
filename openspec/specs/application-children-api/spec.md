# application-children-api Specification

## Purpose
Backend HTTP API for the `Application` child entities — `Reporte`, `Nota`, `Documento`, and `FixData` — providing CRUD endpoints scoped to a parent application, with soft delete and per-application uniqueness/validation rules.
