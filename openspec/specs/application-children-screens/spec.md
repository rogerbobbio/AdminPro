# application-children-screens Specification

## Purpose
Frontend sections on `ApplicationDetail` for the `Application` child entities — Notas, Reportes, Documentos, and FixDatas — providing full CRUD UI (add, edit, delete) for each, backed by the `application-children-api` endpoints.
