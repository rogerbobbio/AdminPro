...
- **THEN** `DashboardComponent` renders inside the app shell with the "Dashboard" nav item highlighted
...
- **THEN** the app navigates to `/proyectos`, which renders a placeholder "Próximamente" page (the real `proyectos` module is out of scope for this change)
