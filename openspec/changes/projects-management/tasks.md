...
- [ ] 3.1 TDD: write a failing test for `CreateProjectCommandValidator` proving a duplicate `Nombre` fails validation (rule PROJECT-001); implement the validator + `CreateProjectCommand`/Handler (insert, set `CreatedAt`/`UpdatedAt`, return new id).
- [ ] 3.2 TDD: write a failing test for `GetProjectsQuery` proving it returns only active projects ordered by `Nombre`, with an `IncludeInactive` flag; implement query + handler + `ProjectSummaryDto`.
- [ ] 3.3 TDD: write a failing test for `GetProjectByIdQuery` proving it returns a `ProjectDetailDto` with `basesDeDatos` populated and throws `NotFoundException` for a missing/inactive id; implement.
