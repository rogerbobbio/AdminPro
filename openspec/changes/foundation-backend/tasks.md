...
- [x] 3.4 TDD: write a failing test proving `LoggingBehavior` logs at `Information` level on entry and exit with the request type name; implement `LoggingBehavior` (rule INF-LOG-001). Verified red (CS0246) then green (20/20 passed).
- [x] 3.5 TDD: write a failing test proving `TransactionBehavior` opens/commits an EF Core transaction for `ICommand` requests and rolls back on exception, and passes through untouched for non-command (query) requests; implement `TransactionBehavior`. Required creating a minimal `AppDbContext` shell in `AdminPro.Infrastructure/Persistence/` (constructor only, no DbSets yet — those arrive in section 4) since `TransactionBehavior` depends on it per `docs/design/DESIGN.md` §2.4. Also fixed `ICommand<TResponse>` (task 3.2) to inherit from `ICommand` — it didn't, so the `request is ICommand` check would have missed every command with a return value. Tested against a real SQLite (`:memory:`) connection since the EF Core InMemory provider doesn't support relational transactions. Verified red (CS0246) then green (23/23 passed).
- [x] 3.6 Register the three behaviors in order (Validation → Logging → Transaction) via an `AddApplicationServices` DI extension method. Verified red (CS0234) then green (24/24 passed) — a test resolving `IPipelineBehavior<FakeRequest, string>` from the container confirms the exact registration order.
...
- [x] 4.2 Implement `AppDbContext` with a `DbSet<T>` for all 11 entities and `ApplyConfigurationsFromAssembly` in `OnModelCreating` (`docs/design/DESIGN.md` §2.3). Required pulling in the `AplicacionServicio` composite-key configuration from task 4.4 immediately — EF Core's model validator fails at first use for any entity without a primary key, so 4.2's own test couldn't pass without it. Ended up implementing all 11 `IEntityTypeConfiguration<T>` classes (task 4.4) together with this task; see 4.4's note. Verified red (CS1061 x11) then green (25/25 passed).
- [x] 4.3 TDD: write a failing test (EF Core InMemory provider) proving a row with `Activo = false` is excluded from a default query but returned with `IgnoreQueryFilters()`; implement the global soft-delete query filter for every entity (rule INF-EF-002). Note: the filter implementation itself landed in 4.4/4.2 out of necessity (see 4.2's note) — this task's test was written after, not before, so it wasn't a true red→green cycle here; it passed immediately (27/27) confirming the filter works as intended.
- [x] 4.4 Add the `IEntityTypeConfiguration<T>` classes for all 11 entities under `Persistence/Configurations/`, including the indexes from rule INF-EF-005 and the cascade-delete rules from rule INF-EF-003. Deviation from `docs/business-rules.md` §2.2.3/§2.2.4 literal "CASCADE DELETE" annotation for `Project -> BaseDeDatos/Application`: implemented as `DeleteBehavior.Restrict` instead, per the more specific and later rule INF-EF-003 in `docs/design/DESIGN.md` ("Soft cascade for Project → Applications/Databases: implemented in Application layer command handler"). Application's own children (Ambiente, Reporte, Nota, Documento, FixData, AplicacionServicio) use hard `DeleteBehavior.Cascade` as specified. `Servicio.ProyectoId` uses `DeleteBehavior.SetNull` per its business-rules.md table note. Verified together with 4.2/4.3 (27/27 passed).
- [x] 4.5 TDD: write a failing test proving `SaveChangesAsync` sets `UpdatedAt` to `UtcNow` on modified entities; implement the `SaveChangesAsync` override (`docs/design/DESIGN.md` §2.3). Verified red (assertion failure: UpdatedAt unchanged) then green (29/29 passed) — also confirmed newly-added entities are NOT touched by the override (handlers set CreatedAt/UpdatedAt explicitly on create, per rule APP-CMD-001).
- [x] 4.6 Generate the initial migration: `dotnet ef migrations add InitialCreate --project backend/src/AdminPro.Infrastructure --startup-project backend/src/AdminPro.Api --output-dir Persistence/Migrations` (`--output-dir` added to match `docs/design/DESIGN.md` §2.1's `Persistence/Migrations/` location; the default is a root-level `Migrations/` folder) — verified the migration contains **no** `HasData`/seed calls. Required installing the `dotnet-ef` global tool, adding `Microsoft.EntityFrameworkCore.Design` to `AdminPro.Api` (required by the tooling on the startup project), and adding an `AppDbContextFactory : IDesignTimeDbContextFactory<AppDbContext>` in `AdminPro.Infrastructure/Persistence/` since `Program.cs` doesn't wire up `AppDbContext` yet (that's task 5.6) — the factory reads `ConnectionStrings:DefaultConnection` from `AdminPro.Api`'s appsettings. Also added the `ConnectionStrings` section to `appsettings.json` (empty placeholder, committed) and `appsettings.Development.json` (real LocalDB connection string, gitignored) ahead of task 5.7, since the factory needed it now.
- [x] 4.7 Apply it locally: `dotnet ef database update --project backend/src/AdminPro.Infrastructure --startup-project backend/src/AdminPro.Api` against `(localdb)\MSSQLLocalDB`; verify with `sqlcmd` that database `AdminPro` exists, all tables exist, and every table has zero rows. Verified: all 11 domain tables exist (Modulos, Projects, BasesDeDatos, Applications, Ambientes, Reportes, Notas, Documentos, FixDatas, Servicios, AplicacionServicios) with 0 rows each; only `__EFMigrationsHistory` (EF Core's internal bookkeeping table) has a row.
